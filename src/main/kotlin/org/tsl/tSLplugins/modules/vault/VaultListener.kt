...
class VaultListener(private val plugin: JavaPlugin, private val manager: VaultManager) : Listener {
    ...
    fun onPlayerInteract(event: PlayerInteractEvent) {
        ...
        try {
            ...
            val checkResult = VaultNBTHelper.checkVault(block, manager.getAbnormalThreshold(), manager.isDebugMode())
            ...
            if (needsEpochReset || needsAbnormalRepair) {
                ...
                if (VaultNBTHelper.repairVault(block, manager.isDebugMode())) {
                    ...
                    if (needsAbnormalRepair) plugin.logger.info("[Vault] 已修复宝库 @ $blockKey (resumesAt: ${checkResult.resumesAt} -> 0)")
                    if (needsEpochReset) plugin.logger.info("[Vault] 已重置周期 @ $blockKey (epoch: $blockEpoch -> $currentEpoch)")
                }
            }
        } finally { manager.releaseLock(blockKey) }
    }
}
