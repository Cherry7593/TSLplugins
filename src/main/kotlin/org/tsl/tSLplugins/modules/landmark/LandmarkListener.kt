...
class LandmarkListener(
    ...
    fun onPlayerQuit(event: PlayerQuitEvent) {
        ...
        compass.cleanupPlayer(event.player.uniqueId)
    }
    ...
    fun onPlayerInteract(event: PlayerInteractEvent) {
        ...
        // 检查是否是地标指南针
        if (!compass.isLandmarkCompass(item)) return
        
        // 右键切换追踪目标
        when (event.action) {
            Action.RIGHT_CLICK_AIR, Action.RIGHT_CLICK_BLOCK -> {
                event.isCancelled = true
                compass.switchTracking(player, true)  // 下一个
            }
            Action.LEFT_CLICK_AIR, Action.LEFT_CLICK_BLOCK -> {
                event.isCancelled = true
                compass.switchTracking(player, false)  // 上一个
            }
            else -> {}
        }
    }
    ...
    fun onItemHeldChange(event: PlayerItemHeldEvent) {
        ...
        // 延迟检查，确保物品已切换
        player.scheduler.runDelayed(plugin, { _ ->
            compass.checkAndStartParticles(player)
        }, null, 1L)
    }
