...
class LandmarkCommand(
    ...
    private val compass: LandmarkCompass
) : SubCommandHandler {
    ...
    private fun handleCompass(player: Player) {
        val compassItem = compass.createCompassItem(player)
        player.inventory.addItem(compassItem)
        player.sendMessage(serializer.deserialize(manager.getMessage("compass_given_self")))
    }
    ...
    /**
     * 处理 unlock 命令
     * /lm unlock <地标名|ALL> <玩家名|ALL>
     */
    private fun handleUnlock(player: Player, args: Array<out String>) {
        if (!player.hasPermission("tsl.landmark.admin")) {
            player.sendMessage(serializer.deserialize(manager.getMessage("no_permission")))
            return
        }
        ...
        if (args.size < 3) {
            player.sendMessage(serializer.deserialize(manager.getMessage("usage_unlock")))
            return
        }

        val landmarkArg = args[1]
        val targetArg = args[2]

        // 获取目标地标列表
        val landmarks: List<Landmark> = if (landmarkArg.equals("ALL", ignoreCase = true)) {
            manager.getAllLandmarks().toList()
        } else {
            val landmark = manager.getLandmark(landmarkArg)
            if (landmark == null) {
                player.sendMessage(serializer.deserialize(
                    manager.getMessage("landmark_not_found", "name" to landmarkArg)
                ))
                return
            }
            listOf(landmark)
        ...
        // 获取目标玩家列表
        val targetPlayers: List<org.bukkit.OfflinePlayer> = if (targetArg.equals("ALL", ignoreCase = true)) {
            Bukkit.getOnlinePlayers().toList()
        } else {
            val target = Bukkit.getPlayerExact(targetArg)
                ?: Bukkit.getOfflinePlayerIfCached(targetArg)
            if (target == null) {
                player.sendMessage(serializer.deserialize("&c玩家 $targetArg 未找到（需曾经登录过服务器）"))
                return
            }
            listOf(target)
        }
        ...
        for (target in targetPlayers) {
            for (landmark in landmarks) {
                if (manager.storage.unlockLandmark(target.uniqueId.toString(), landmark.id)) {
                    totalUnlocked++
                }
            }
        }
        ...
        // 发送结果消息
        val landmarkDesc = if (landmarkArg.equals("ALL", ignoreCase = true)) "所有地标" else landmarkArg
        val playerDesc = if (targetArg.equals("ALL", ignoreCase = true)) "所有在线玩家" else targetArg
        
        player.sendMessage(serializer.deserialize(
            manager.getMessage("unlock_batch_success",
                "landmark" to landmarkDesc,
                "player" to playerDesc,
                "count" to totalUnlocked.toString())
        ))
    }

    /**
     * 处理 lock 命令
     * /lm lock <地标名|ALL> <玩家名|ALL>
     */
    private fun handleLock(player: Player, args: Array<out String>) {
        if (!player.hasPermission("tsl.landmark.admin")) {
            player.sendMessage(serializer.deserialize(manager.getMessage("no_permission")))
            return
        }
        ...
        if (args.size < 3) {
            player.sendMessage(serializer.deserialize(manager.getMessage("usage_lock")))
            return
        }
        ...
        // 获取目标地标列表（排除默认解锁的）
        val landmarks: List<Landmark> = if (landmarkArg.equals("ALL", ignoreCase = true)) {
            manager.getAllLandmarks().filter { !it.defaultUnlocked }
        } else {
            val landmark = manager.getLandmark(landmarkArg)
            if (landmark == null) {
                player.sendMessage(serializer.deserialize(
            ...
            if (landmark.defaultUnlocked) {
                player.sendMessage(serializer.deserialize(
                    manager.getMessage("lock_default_unlocked", "landmark" to landmarkArg)
                ))
                return
            }
            listOf(landmark)
        ...
        for (target in targetPlayers) {
            for (landmark in landmarks) {
                if (manager.storage.lockLandmark(target.uniqueId.toString(), landmark.id)) {
                    totalLocked++
                }
            }
        }
        ...
        player.sendMessage(serializer.deserialize(
            manager.getMessage("lock_batch_success",
                "landmark" to landmarkDesc,
                "player" to playerDesc,
                "count" to totalLocked.toString())
        ))
    }
    ...
    private fun showHelp(player: Player) {
        ...
        player.sendMessage(serializer.deserialize("&e/lm compass &7- 获取导航指南针"))
        if (player.hasPermission("tsl.landmark.admin")) {
            ...
            player.sendMessage(serializer.deserialize("&e/lm unlock <地标|ALL> <玩家|ALL> &7- 解锁地标"))
            player.sendMessage(serializer.deserialize("&e/lm lock <地标|ALL> <玩家|ALL> &7- 锁定地标"))
        }
    }
    ...
    override fun tabComplete(sender: CommandSender, command: Command, label: String, args: Array<out String>): List<String> {
        ...
        return when (args.size) {
            1 -> {
                val suggestions = mutableListOf("gui", "list", "tp", "info", "help", "compass")
                if (sender.hasPermission("tsl.landmark.admin")) {
                    suggestions.addAll(listOf("opk", "create", "delete", "edit", "setwarp", "trust", "untrust", "unlock", "lock"))
                }
            ...
            2 -> {
                when (args[0].lowercase()) {
                    ...
                    "unlock", "lock" -> {
                        ...
                        suggestions.addAll(manager.getAllLandmarks().map { it.name })
            ...
            3 -> {
                when (args[0].lowercase()) {
                    ...
                    "unlock", "lock" -> {
                        ...
                        suggestions.filter { it.startsWith(args[2], ignoreCase = true) }
                    }
                    else -> emptyList()
                }
            }
