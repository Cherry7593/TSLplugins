...
class VaultManager(private val plugin: JavaPlugin) {
    ...
    fun loadConfig() {
        ...
        if (enabled) {
            plugin.logger.info("[Vault] 已启用 - 周期: $currentEpoch, 间隔: ${resetIntervalDays}天")
        }
    }
    ...
    fun checkAndUpdateEpoch(): Boolean {
        ...
        if (System.currentTimeMillis() >= lastResetTimestamp + intervalMs) {
            ...
            lastResetTimestamp = System.currentTimeMillis()
            saveEpochConfig()
            plugin.logger.info("[Vault] 周期更新: $currentEpoch")
            return true
        }
        return false
    }
    ...
    fun tryAcquireLock(blockKey: String): Boolean {
        ...
        processingLocks.compute(blockKey) { _, existing ->
            if (existing == null || now - existing >= lockTimeoutMs) {
                acquired = true
                now
            } else {
                existing
    ...
    fun releaseLock(blockKey: String) {
        processingLocks.remove(blockKey)
    }
    ...
    fun debug(message: String) {
        if (debugMode) plugin.logger.info("[Vault/Debug] $message")
    }

    fun reload() {
        loadConfig()
        plugin.logger.info("[Vault] 配置已重载")
    }
}
